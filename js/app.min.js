...
! function($) {
    var Projects = {
        init: function(options, elem) {
			...
			self.options = $.extend({}, $.fn.projects.options, options), 
			self.elem = $(elem), 
			self.container = self.elem.find("div.con-box"), 
			self.boxes = self.container.find("a.box"), 
			self.zeros = self.boxes.find("div.grid-zero"), 
			self.track = self.elem.find("div.zone"), 
			self.piece = self.elem.find("div.cursor"), 
			self.index = 0, 
			self.length = self.boxes.size(), 
			self.zone = self.elem.find("div.zone-text").find("span").eq(0), 
			self.ratio = 1800 / 1184, 
			self.container.css("width", 25 * self.length + "%"), 
			self.boxes.css("width", 100 / self.length + "%"), 
			self.width = self.boxes.eq(0).width() / 2, 
			self.height = self.viewport.height() / 2, 
			self.a = [], 
			self.b = [], 
			self.c = [], 
			self.d = [], 
			self.boxes.each(function() {
				...
				self.boxes.find("path").attr("d", "M " + (self.width - 10) + " " + (self.height - 60) + " L " + (self.width + 10) + " " + (self.height - 60) + " L " + (self.width + 10) + " " + (self.height - 59) + " L " + (self.width - 10) + " " + (self.height - 59) + " Z"), 
				self.buttons = self.elem.find("button.prev, button.next").eq(0).css({
					opacity: 0
				}).end(),
				self.linePrev = self.elem.find("svg.line-prev"), 
				self.lineNext = self.elem.find("svg.line-next"), 
				self.e = {
					x: 1,
					y: 19
				}, self.f = {
